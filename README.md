...
[GATE: APROVAÇÃO DO BRIEFING]
          ...
          ├── imagem ─────→ [GPT-IMAGE2-UNITY] → default (OAuth ChatGPT)
...
| Geração de carrossel | `duduesh/carrossel-unity` | Texto → HTML → PNG via Playwright, 3 estilos, Instagram/TikTok |
...
| Imagem (default) | `duduesh/gpt-image2-unity` | GPT Image 2 via OAuth ChatGPT — sem custo adicional; latência 60–180s |
...
**O que falta construir:** skill `briefing-unity` (MVP) + gates de aprovação + skill `roteiro-unity` (V1).
...
| Briefing orchestrator | `briefing-unity` | 🔨 MVP |
| Gates de aprovação | Fluxo conversacional no Claude Code | 🔨 MVP |
...
| Fase | O que entrega |
|---|---|
| **MVP** | `briefing-unity` + gates de aprovação. 10 conteúdos como validação. |
...
|---|---|---|
| `GEMINI_API_KEY` | Google Gemini | Grátis — [aistudio.google.com](https://aistudio.google.com) |
...
| `FAL_KEY` | FAL API | Pago — usar apenas como contingência |

**OpenAI:** token gerado via `npx @openai/codex login`, salvo em `~/.codex/auth.json` — não vai para o `.env`.
